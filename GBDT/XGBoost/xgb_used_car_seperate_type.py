...
params_xgb = {'num_leaves': 256, # adjust
          'min_child_weight': 40, # adjust 
          'max_depth': 8, # adjust
          'eta': 0.3,
          'gamma':0.5,
...
out_xgb = pd.DataFrame([],index=X_os.index,columns=['OS_predicted_price'])
is_xgb = pd.DataFrame([],index=X.index,columns=['IS_predicted_price'])
...
for t in split_type_list:
    ...
    if len(X_type)<5000:
        continue
    result_dict_xgb = train_model_regression(X=X_type, X_test=X_os_type, y=y.iloc[X_type_index], params=params_xgb, folds=folds,  eval_metric='mse', verbose=300, early_stopping_rounds=100)
    out_xgb['OS_predicted_price'].iloc[X_os_type_index] = result_dict_xgb['prediction']
    is_xgb['IS_predicted_price'].iloc[X_type_index] = result_dict_xgb['oof']

# fill nan with original prediction
whole_model_prediction_is = pd.read_csv(f'{file_folder}/output/xgb/IS_prediction_xgb_10,20.csv')
whole_model_prediction_os = pd.read_csv(f'{file_folder}/output/xgb/OS_prediction_xgb_10,20.csv')

out_xgb['OS_predicted_price'][out_xgb['OS_predicted_price'].isnull()] = whole_model_prediction_os['OS_predicted_price'][out_xgb['OS_predicted_price'].isnull()]
is_xgb['IS_predicted_price'][is_xgb['IS_predicted_price'].isnull()] = whole_model_prediction_is['IS_predicted_price'][is_xgb['IS_predicted_price'].isnull()]


print("Writing files...")
out_xgb.to_csv(f'{file_folder}/output/OS_prediction_xgb_split_'+split_col+'1.csv', index=False)
is_xgb.to_csv(f'{file_folder}/output/IS_prediction_xgb_split_'+split_col+'1.csv', index=False)
print("len(OS)",len(out_xgb))
print("len(IS)",len(is_xgb))
print("Null OS",out_xgb.isnull().any())
print("Null IS",is_xgb.isnull().any())
