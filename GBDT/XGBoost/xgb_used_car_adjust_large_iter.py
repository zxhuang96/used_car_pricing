...
def train_model_regression(X, X_test, y, params, folds, eval_metric='mae', columns=None,  verbose=500, early_stopping_rounds=200):
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X)):
        ...
        model = xgb.train(dtrain=train_data, num_boost_round=400, evals=watchlist, early_stopping_rounds=early_stopping_rounds, verbose_eval=verbose, params=params)
...
X['median_income']=(X['median_income']-X['median_income'].mean())/X['median_income'].std()
X['manufacturer_mean_price']=(X['manufacturer_mean_price']-X['manufacturer_mean_price'].mean())/X['manufacturer_mean_price'].std()
X['manufacturer_make_mean_price']=(X['manufacturer_make_mean_price']-X['manufacturer_make_mean_price'].mean())/X['manufacturer_make_mean_price'].std()
X['income_rank']=X['income_rank']/X['income_rank'].max()

X_os = data_all_OS[features_colname]

#scaling
X_os['odometer']=(X_os['odometer']-X['odometer'].mean())/X['odometer'].std()
X_os['median_income']=(X_os['median_income']-X['median_income'].mean())/X['median_income'].std()
X_os['manufacturer_mean_price']=(X_os['manufacturer_mean_price']-X['manufacturer_mean_price'].mean())/X['manufacturer_mean_price'].std()
X_os['manufacturer_make_mean_price']=(X_os['manufacturer_make_mean_price']-X['manufacturer_make_mean_price'].mean())/X['manufacturer_make_mean_price'].std()
X_os['income_rank']=X_os['income_rank']/X['income_rank'].max()
...
out_xgb.to_csv(f'{file_folder}/output/OS_prediction_xgb_3.csv', index=False)

is_xgb = pd.DataFrame([])
is_xgb['IS_predicted_price'] = result_dict_xgb['oof']
is_xgb.to_csv(f'{file_folder}/output/IS_prediction_xgb_3.csv', index=False)
