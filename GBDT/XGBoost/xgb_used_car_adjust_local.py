...
def train_model_regression(X, X_test, y, params, folds, eval_metric='mae', columns=None,  verbose=500, early_stopping_rounds=200):
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X)):
        ...
        model = xgb.train(dtrain=train_data, num_boost_round=300, evals=watchlist, early_stopping_rounds=early_stopping_rounds, verbose_eval=verbose, params=params)
...
params_xgb = {'num_leaves': 500, # adjust
          'min_child_weight': 60, # adjust 
          'max_depth': 10, # adjust
          'eta': 0.2,
          'gamma':0.5,
...
out_xgb.to_csv(f'{file_folder}/output/OS_prediction_xgb_10,60.csv', index=False)

is_xgb = pd.DataFrame([])
is_xgb['IS_predicted_price'] = result_dict_xgb['oof']
is_xgb.to_csv(f'{file_folder}/output/IS_prediction_xgb_10,60.csv', index=False)
