...
result_dict_lgb = train_model_regression(X=X, X_test=X_os, y=y, params=params_lgb, folds=folds,  eval_metric='mse', verbose=500, early_stopping_rounds=200, n_estimators=2000)

print("Writing files...")
out_lgb = pd.DataFrame([])
out_lgb['OS_predicted_price'] = result_dict_lgb['prediction']
out_lgb.to_csv(f'{file_folder}/output/OS_prediction_lgb_10,100.csv', index=False)

is_lgb = pd.DataFrame([])
is_lgb['IS_predicted_price'] = result_dict_lgb['oof']
is_lgb.to_csv(f'{file_folder}/output/IS_prediction_lgb_10,100.csv', index=False)
