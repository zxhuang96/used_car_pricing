...
# scaling
X_os = utils.scaling(X_os,data_all_OS,zscore_cols,positive_cols)
...
out_lgb.to_csv(f'{file_folder}/output/OS_prediction_lgb_additional_feature.csv', index=False)

is_lgb = pd.DataFrame([])
is_lgb['IS_predicted_price'] = result_dict_lgb['oof']
is_lgb.to_csv(f'{file_folder}/output/IS_prediction_lgb_additional_feature.csv', index=False)
