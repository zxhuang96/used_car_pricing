...
n_fold = 5
folds = KFold(n_splits=n_fold, shuffle=True, random_state=13)
params_cat = {'depth': 6,
          'learning_rate': 0.2,
          "random_seed": 13,
          'reg_lambda': 5,
...
'''
params_cat_smaller_depth = {'depth': 4,
          'learning_rate': 0.2,
          "random_seed": 13,
          'reg_lambda': 5,
          'thread_count': -1,
...
'''
split_col = 'k_cluster_manufacturer_10_cluster'

X, X_os = utils.create_cluster(X, X_os, ['manufacturer_mean_price'], 10, max_iter=300, n_init=10, outname=split_col)


split_type_list = list(X[split_col].unique())
if np.nan in split_type_list:
    split_type_list.remove(np.nan)

out_cat = pd.DataFrame([],index=X_os.index,columns=['OS_predicted_price'])
is_cat = pd.DataFrame([],index=X.index,columns=['IS_predicted_price'])

for t in split_type_list:
    print("*"*30)
    print("Type "+str(t)+": "+str(split_type_list.index(t)+1)+"/"+str(len(split_type_list)))
    X_type = X[X[split_col]==t]
    X_os_type = X_os[X[split_col] ==t]
    ...
    '''
    if len(X_type)>20000:
        result_dict_cat = train_model_regression(X=X_type, X_test=X_os_type, y=y.iloc[X_type_index], params=params_cat, folds=folds,  eval_metric='mae', iterations=1000)
    else:
    '''
    result_dict_cat = train_model_regression(X=X_type, X_test=X_os_type, y=y.iloc[X_type_index], params=params_cat, folds=folds,  eval_metric='mae', iterations=1000)
    out_cat['OS_predicted_price'].iloc[X_os_type_index] = result_dict_cat['prediction']
    is_cat['IS_predicted_price'].iloc[X_type_index] = result_dict_cat['oof']

# fill nan with original prediction
whole_model_prediction_is = pd.read_csv(f'{file_folder}/output/IS_prediction_cat_1.csv')
whole_model_prediction_os = pd.read_csv(f'{file_folder}/output/OS_prediction_cat_1.csv')

out_cat['OS_predicted_price'][out_cat['OS_predicted_price'].isnull()] = whole_model_prediction_os['OS_predicted_price'][out_cat['OS_predicted_price'].isnull()]
is_cat['IS_predicted_price'][is_cat['IS_predicted_price'].isnull()] = whole_model_prediction_is['IS_predicted_price'][is_cat['IS_predicted_price'].isnull()]

print("Writing files...")
out_cat.to_csv(f'{file_folder}/output/OS_prediction_cat_kmeans_'+split_col+'1.csv', index=False)
is_cat.to_csv(f'{file_folder}/output/IS_prediction_cat_kmeans_'+split_col+'1.csv', index=False)
print("len(OS)",len(out_cat))
print("len(IS)",len(is_cat))
print("Null OS",out_cat.isnull().any())
print("Null IS",is_cat.isnull().any())
